...
def generate_launch_description():
    ...
    dashgo_driver_config = os.path.join(
        get_package_share_directory('dashgo_driver'), 'config', 'my_dashgo_params.yaml'
    )
    ...
    imu_mux_config = os.path.join(
        get_package_share_directory('ir_sensor'), 'param', 'mux_plus.yaml'
    )
    ...
    return LaunchDescription([
        ...
        # Command velocity multiplexer (cmd_vel_mux) node
        Node(
            package='nodelet',
            executable='nodelet',
            name='cmd_vel_mux',
            arguments=['load', 'yocs_cmd_vel_mux/CmdVelMuxNodelet', 'vel_mux_manager'],
            parameters=[{'yaml_cfg_file': imu_mux_config}],
            remappings=[('cmd_vel_mux/output', 'smoother_cmd_vel')]
        ),
