...
class Arduino:
    ...
    def execute_ack(self, cmd):
        ...
        try:
            ...
            # print (f"CMD:" + cmd)
            ack = self.recv(self.timeout)
            # print(f"Received: {ack if ack is not None else 'None'}")  # Tránh lỗi nếu ack là None
...
class BaseController(Node):
    def __init__(self, arduino, base_frame):
        ...
        self.get_logger().info(f"{self.wheel_diameter}")
        self.get_logger().info(f"{self.gear_reduction}")
        self.get_logger().info(f"{self.encoder_resolution}")
        self.ticks_per_meter = self.encoder_resolution * self.gear_reduction  / (self.wheel_diameter * PI)
        self.get_logger().info(f"{self.ticks_per_meter}")
        ...
        self.create_subscription(Twist,"smoother_cmd_vel",self.cmdVelCallback, 10)
        self.get_logger().info(f"Pass sub")
        ...
        self.get_logger().info(f"Pass all BaseController")
    ...
    def poll(self):
        ...
        # dt = (now.sec - self.then.sec) + (now.nanosec - self.then.nanosec) / 1e9
        # self.then = now
        ...
        if self.enc_left == None:
            ...
            if self.ticks_per_meter == 0:
                self.get_logger().error("Ticks per meter is zero, cannot compute dleft.")
                dleft = 0  # hoặc giá trị mặc định phù hợp
            else:
                dleft = 1.0 * (left_enc + self.l_wheel_mult * (self.encoder_max - self.encoder_min)-self.enc_left) / self.ticks_per_meter
        ...
        if not self.stopped:
            ...
            self.get_logger().info(f"Driving Robot")

        self.t_next = now + self.t_delta
        # self.t_next = now + self.t_delta.to_msg()
    ...
    def cmdVelCallback(self, req):
        ...
        self.get_logger().info(f"Handling Keyboard")
...
class ArduinoROS(Node):
    def __init__(self):
        ...
        self.use_base_controller = self.declare_parameter("use_base_controller", False).value
        ...
        # self.controller.drive(49,49)    #i
        # self.controller.drive(-49,-49)  #,
        # self.controller.drive(-32,-67)  #m
        # self.controller.drive(32,67)    #u
        # self.controller.drive(-67,-32)  #.
        # self.controller.drive(67,32)    #o
        # self.controller.drive(-17,17)   #j
        # self.controller.drive(17,-17)   #l
